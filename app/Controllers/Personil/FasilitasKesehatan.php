...
class FasilitasKesehatan extends BaseController
{
	...
	public function detail($place_id)
	{
		...
		return view('personil/fasilitas-kesehatan/detail', $data);
	}
	...
	public function getNearbyLocationFaskes()
	{
		...
		$fetch = file_get_contents("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=$koordinat&radius=$radius&type=$poi&sensor=true&key=$this->keyAPI&region=ID&language=id");
		$json = json_decode($fetch);
	...
	public function getDetailFaskes()
	{
		...
		$fetch = file_get_contents("https://maps.googleapis.com/maps/api/place/details/json?placeid=$place_id&key=$this->keyAPI&region=ID&language=id");
		$json = json_decode($fetch);
