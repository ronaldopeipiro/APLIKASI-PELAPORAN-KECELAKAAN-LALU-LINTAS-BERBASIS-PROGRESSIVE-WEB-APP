									...
									<table class="table table-responsive table-hover table-transparent table-vcenter" id="data-table-custom">
										...
										<tbody>
											...
											<?php foreach ($personil as $row) : ?>
												...
												if ($row['foto'] == '') {
													$foto =	base_url() . "/img/noimg.png";
												} else {
													$foto = base_url() . "/img/personil/" . $row['foto'];
												}

												$id_pangkat = $row['id_pangkat'];
												if ($id_pangkat != "") {
													$pangkat = $db->query("SELECT * FROM tb_pangkat_personil WHERE id_pangkat = '$id_pangkat'")->getRow();

													$pangkat_personil = $pangkat->singkatan;
												} else {
													$pangkat_personil = "";
												}
												?>
												...
												<tr>
													...
													<td style="vertical-align: middle;" class="text-center">
														<img src="<?= $foto; ?>" style="width: 57px; height: 57px; object-fit: cover; object-position: center; border-radius: 10px; border: solid 2px #ddd;">
													</td>
													<td style="vertical-align: middle;" class="text-left">
														<?= $pangkat_personil . " " . $row['nama_lengkap']; ?>
													</td>
													<td style="vertical-align: middle;" class="text-left">
														<?= $row['email']; ?>
													</td>
													...
													<td style="vertical-align: middle;">
														<div class="list-unstyled d-flex">
															<li style="margin-right: 5px;">
																<button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#modal-detail-pelapor" data-foto="<?= $foto; ?>" data-namalengkap="<?= $pangkat_personil . ' ' . $row['nama_lengkap']; ?>" data-nrp="<?= $row['nrp']; ?>" data-email="<?= $row['email']; ?>" data-nohp="<?= $row['no_hp']; ?>" data-status="<?= $text_status; ?>" data-latitude="<?= $row['latitude']; ?>" data-longitude="<?= $row['longitude']; ?>" data-createdatetime="<?= strftime('%d/%m/%Y %H:%M:%S WIB', strtotime($row['create_datetime'])); ?>" data-updatedatetime="<?= strftime('%d/%m/%Y %H:%M:%S WIB', 	strtotime($row['update_datetime'])); ?>" data-lastlogin="<?= strftime('%d/%m/%Y %H:%M:%S WIB', strtotime($row['last_login'])); ?>" title="Detail">
				...
				<div class="row">
					<div class="col-lg-6">
						...
						<table class="table-sm table-borderless table-responsive mt-3">
							<tr>
								<td style="width: 130px;">Pangkat / Nama Lengkap</td>
								<td>:</td>
								<td>
