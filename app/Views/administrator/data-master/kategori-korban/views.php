								...
								<div class="d-flex justify-content-between align-items-center w-100">
									<div class="">
										<div class="d-flex justify-content-between align-items-center">
											...
											<span>Data Kategori Korban</span>
										</div>
									</div>
							...
							<div class="card-body">
								...
								<div id="table-content">
									<table class="table table-responsive table-mobile-lg table-hover table-bordered table-vcenter" id="data-table-custom" style="font-size: 12px;">
										...
										<tbody>
											<?php $no = 1; ?>
											<?php foreach ($kategori_korban as $row) : ?>
												<tr>
													...
													<td style="vertical-align: middle;" clas>
														<div class="list-unstyled d-flex justify-content-center">
															<li class="me-2">
																<a title="Ubah Data" data-title="Ubah Data" data-idkategorikorban="<?= $row['id_kategori_korban']; ?>" data-kategorikorban="<?= $row['kategori_korban']; ?>" data-deskripsi="<?= $row['deskripsi']; ?>" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modalFormTambahUbah">
															...
															<li>
																<form id="formHapusData-<?= $no ?>">
																	<?= csrf_field(); ?>
																	<input type="hidden" name="id_kategori_korban" value="<?= $row['id_kategori_korban']; ?>">
																...
																<script>
																	$("#formHapusData-<?= $no ?>").submit(function(e) {
																		...
																		var id_kategori_korban = $(this).find("input[name='id_kategori_korban']").val();

																		$.ajax({
																			type: "POST",
																			url: "<?= base_url() ?>/Admin/DataMaster/hapus_kategori_korban",
																			dataType: "JSON",
																			data: {
		...
		<div class="modal-content">
			<form id="formTambahUbahData">
				...
				<div class="modal-body">
					...
					<input type="hidden" name="id_kategori_korban" id="id_kategori_korban">

					<div class="form-group mb-3">
						<label for="kategori_korban" class="col-form-label col-sm-12">
							Kategori
						</label>
						<div class="col-sm-12">
							<input type="text" name="kategori_korban" id="kategori_korban" class="form-control" placeholder="Masukkan kategori ...">
						</div>
					</div>
	...
	$(document).ready(function() {
		...
		$("#modalFormTambahUbah").on("show.bs.modal", function(event) {
			...
			if (title == "Tambah Data") {
				...
				var kategorikorban = button.data("kategorikorban");
				var deskripsi = button.data("deskripsi");

				$(this).find("#action").val("ubah");
				$(this).find("#id_kategori_korban").val(idkategorikorban);
				$(this).find("#kategori_korban").val(kategorikorban);
	...
	$(document).ready(function() {
		$(function() {
			...
			$("#formTambahUbahData").submit(function(e) {
				...
				if (action == "tambah") {
					var kategori_korban = $('#kategori_korban').val();
					var deskripsi = $('#deskripsi').val();

					$.ajax({
						type: "POST",
						url: "<?= base_url() ?>/Admin/DataMaster/tambah_kategori_korban",
						dataType: "JSON",
						data: {
					...
					var id_kategori_korban = $('#id_kategori_korban').val();
					var kategori_korban = $('#kategori_korban').val();
					var deskripsi = $('#deskripsi').val();

					$.ajax({
						type: "POST",
						url: "<?= base_url() ?>/Admin/DataMaster/ubah_kategori_korban",
						dataType: "JSON",
						data: {
							id_kategori_korban: id_kategori_korban,
							kategori_korban: kategori_korban,
